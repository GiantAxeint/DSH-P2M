...
export function decide(conflict, { order, self, core, cfg }) {
  ...
  const advice = conflict.advice ?? null
  ...
  // 结构性冲突（C3/C4/C5/C7）：配置层/服务面打架，需要人来改，v1 一律只报告
  if (
    conflict.kind === KIND.CONFIG_CLASH
    || conflict.kind === KIND.DUP_ID
    || conflict.kind === KIND.DUP_NAME
    || conflict.kind === KIND.SERVICE_CLASH
  ) {
    return {
      action: 'report',
      loserId: null,
  ...
  if (conflict.kind === KIND.AUTO_DISABLED || conflict.kind === KIND.BOOT_CRASH) {
    if (!cfg.autoIsolate) {
      return { action: 'report', loserId: conflict.entryId, advice, reason: 'autoIsolate disabled; reported only' }
    }
    const loser = related.find((e) => e.id === conflict.entryId) ?? order.find((e) => e.id === conflict.entryId)
    if (!loser) return { action: 'ignore', loserId: null, advice, reason: 'offender no longer in tree' }
    return {
      action: 'isolate',
      loserId: loser.id,
      advice,
  ...
  // 其他（C6 等）保守处理：报告
  return { action: 'report', loserId: null, advice, reason: `kind ${conflict.kind} not auto-actionable; reported` }
}
