...
import { scanRuntime, scanStatic, scanServiceClashes, KIND, resolvePkgRoot } from './conflicts.js'
...
const DEFAULTS = {
  ...
  scanBootLayers: true, // E4：boot 时对全补丁层跑一轮静态扫描，只记录 error/带建议的冲突
  patchLayers: [], // 供 scanConflicts() 读取的补丁文件路径（可选；默认自动发现，见 loadPatchLayers）
...
export function apply(ctx, config = {}) {
  ...
  // ---- 日志分级与显示（README「日志显示规则」；DESIGN §15）----
  //  [WARNING] 黄 = 提示类（不中断 DSH 启动，但需关注）
  //  [ERROR]   红 = 中断类 / p2m 内部异常类
  //  warning/error 尾部一律附加全英文检查指引（应检查的文件/路径/冲突细节）
  //  颜色：仅 stderr 为 TTY 且未设 NO_COLOR 时输出 ANSI；cfg.logColor='always'|'never' 可强制
  const ansiColor = (() => {
    const mode = cfg.logColor || 'auto'
    if (mode === 'always') return { y: '\x1b[33m', r: '\x1b[31m', z: '\x1b[0m' }
    if (mode === 'never') return { y: '', r: '', z: '' }
    if (process.env.NO_COLOR !== undefined) return { y: '', r: '', z: '' }
    const tty = Boolean(process.stderr && process.stderr.isTTY)
    return tty ? { y: '\x1b[33m', r: '\x1b[31m', z: '\x1b[0m' } : { y: '', r: '', z: '' }
  })()
  const log = (...args) => console.log(`[p2m] ${args.join(' ')}`)
  const warn = (...args) => console.warn(`${ansiColor.y}[WARNING]${ansiColor.z} [p2m] ${args.join(' ')}`)
  const error = (...args) => console.error(`${ansiColor.r}[ERROR]${ansiColor.z} [p2m] ${args.join(' ')}`)
  // warning/error 的从属细节行：缩进、不重复前缀（避免 [WARNING] 逐行刷屏）
  const detail = (...args) => console.warn(`    ${args.join(' ')}`)
  // p2m 内部异常：归 [ERROR] 红（区别于外部插件冲突的 [WARNING] 提示），附全英文上报指引
  const reportInternal = (e) => {
    error(`internal error: ${errText(e)}`)
    detail('hint: This is an internal p2m failure, not a plugin conflict — please report the message above to the DSH-P2M maintainers.')
  }
  ...
  const safe = (fn, fallback = null) => {
    try {
      ...
      if (result && typeof result.then === 'function') {
        return result.catch((error) => { reportInternal(error); return fallback })
      }
      return result
    } catch (error) {
  ...
  const rec = { state: null, ledgerEntries: {}, lastLedgerWrite: 0, guardIds: [], forceUntil: {}, c7: { at: 0, sig: '', conflicts: [] }, reportedKeys: new Set(), resolvePrev: null, lastResolveWriteAt: 0, patchLayersCache: null }
  const canGate = () => cfg.autoIsolate && cfg.autoGateRisk

  // ---------- E4：自动发现补丁层（profile patch + 各 bundle 的 cordis.patch.yml） ----------
  const loadPatchLayers = () => {
    if (rec.patchLayersCache) return rec.patchLayersCache
    const out = []
    const seen = new Set()
    const push = (p) => {
      const key = path.resolve(p) // 归一化分隔符，防同一文件因 / 与 \ 混写重复入列
      if (!seen.has(key) && fs.existsSync(p)) { seen.add(key); out.push(p) }
    }
    push(path.join(baseDir, 'cordis.patch.yml'))
    try {
      const profilePkg = JSON.parse(fs.readFileSync(path.join(baseDir, 'package.json'), 'utf8'))
      for (const spec of Object.keys(profilePkg.dependencies ?? {})) {
        const root = resolvePkgRoot(spec, baseDir)
        if (root) push(path.join(root, 'cordis.patch.yml'))
      }
    } catch { /* profile package.json 不可读 → 只有自身 patch */ }
    rec.patchLayersCache = out
    return out
  }
  const readLayers = (files) => files.map((f) => ({ name: f, text: fs.readFileSync(f, 'utf8') }))
  ...
  const recordResolveDrift = (next, entries) => {
    ...
    if (drift.length) {
      ...
      if (logOnce('resolve-drift', drift.map((d) => d.entryId).join(','), summary)) {
        ...
        warn(`resolve drift: ${drift.length} entry(s) changed resolution — ${summary}`)
        detail(`hint: Check ${incidentFile} (kind=resolve-drift) and ${stateFile} (#lastResolve) for per-spec before/after details; run "pnpm install" inside the profile to restore local resolutions.`)
      }
    }
  ...
  const runPreflight = () => {
    ...
    if (errors.length) {
      warn(`preflight: ${errors.length} peer violation(s) — 建议先按下方 fix 命令钉版本，再重启 DSH`)
      for (const e of errors.slice(0, 5)) detail(`${e.bundle} peer ${e.peer}: declared ${e.declared}, resolved ${e.resolved ?? '(none)'} | fix: ${e.fix}`)
      detail(`hint: Check the peer ranges declared in ${path.join(baseDir, 'package.json')} against the resolved versions above, then run the fix command (pnpm add ...) inside the profile before restarting DSH.`)
    ...
    for (const conflict of conflicts) {
      ...
      if (protectedHit) {
        ...
        if (now() > cooldown) {
          ...
          error(`protected entry "${conflict.entryId}" was auto-disabled by the loader — attempting hot restore`)
          detail(`hint: Check ${guardFile} and the loader tree to find what disabled it; protected entries (DSH core / p2m) must never be isolated.`)
  ...
  const api = {
    ...
    scanConflicts: () => safe(() => {
      const files = (cfg.patchLayers && cfg.patchLayers.length) ? cfg.patchLayers : loadPatchLayers()
      const layers = files
  ...
  const boot = safe(() => {
    ...
    if (poisoned.length) {
      ...
      detail(`hint: Check ${guardFile} — it contained p2m/DSH-core entries and they were removed automatically; review what wrote those rows before editing the guard manually.`)
    }
    refreshGuardIds()
    ...
    // E4：boot 静态扫描全补丁层，只记录 error 或带修复建议的冲突（含“出厂禁用被覆盖”形态）
    if (cfg.scanBootLayers) {
      safe(() => {
        const hits = scanStatic(readLayers(loadPatchLayers()))
        const meaningful = hits.filter((c) => c.severity === 'error' || c.advice)
        if (meaningful.length) {
          log(`boot static scan: ${hits.length} conflict(s), ${meaningful.length} actionable`)
          for (const c of meaningful) {
            if (logOnce(`static-scan|${c.kind}`, c.entryId, c.detail)) {
              incident({ kind: 'static-conflict', entryId: c.entryId ?? null, conflictKind: c.kind, detail: c.detail, advice: c.advice ?? null })
              warn(`static ${c.kind} ${c.entryId ?? ''}: ${c.detail}${c.advice ? ` | 建议: ${c.advice}` : ''}`)
              detail(`hint: See ${incidentFile} (kind=static-conflict) for the full record and inspect the discovered patch layers (profile + bundle cordis.patch.yml) listed under state.json#patchLayersCache for the conflicting rows.`)
            }
          }
        }
      })
  ...
  if (!loader) {
    ...
    detail(`hint: Check that p2m is loaded as a manager entry by DSH (see ${path.join(baseDir, 'cordis.patch.yml')} / plugins config); reporting-only mode disables runtime isolation and auto-disable.`)
  }
